...
int CRvSender::SendRvMsg( CString cszToSubject, CString cszSendMsg, int ervSendType, CString cszSendMsgBinary, CString cszSendMsgPPBODY, CString cszSendMsgDMS )
{
    ...
    {
        ervSendType |= MESSAGE;         // Text 문자열 값이 있으면 MESSAGE 추가 한다.
    }
...
//          if ( mainLog != NULL ) mainLog->WriteTC(eLOG_TYPE_DEBUG, eLOG_IFS_NONE, eTAB1, "(CRvListener::onMsg) :: ..... RV Read DMS Data. Length=%d",(unsigned int) msgField.getSize()  );
        ...
        // MESO 에서 AF_INFO, AF_CNT Field 를 사용하고 있어, DATA 부분을 처리하지 못하는 문제가 있어 return 하지 않도록 함
        /*else 
        {
            if ( mainLog != NULL ) mainLog->WriteTC(eLOG_TYPE_ERROR, eLOG_IFS_NONE, eTAB1, "(CRvListener::onMsg) :: received   Not Defined RV FIELD  Name [%s] subject [%s] reply subject [%s] ", 
                                                        cszFieldName.String(), sendSubject, replySubject );
    ...
    if ( m_BlockMode == RV_MSG_NON_BLOCK )
    {
        ...
        }else
        {
            ...
            while( 1 )
            {
                ...
                if ( status != TIBRV_OK && status != TIBRV_TIMEOUT )   // Error 발생한 경우 Break 처리
                {
                ...
                if ( m_rvRead == TRUE )   // Rv Message 수신한 경우 Break 처리
                {
                    break;
                }
                ...
                if ( d_diff > nTimeout )   // Timer 시간 Over시 Error 처리
                {
                    cszRvMsg    = "RV ERROR :: TIMEOUT"; // Error 메시지 리턴
                    return FALSE;
                }
...
//Debug("GetRvMessage ::: dispatch()....end!!!");
    ...
    if ( m_rvRead == TRUE )  
    {
        ...
        if ( eRvReadType & MESSAGE )  cszRvMsg    = m_rvmessage; // 메세지와 type만 리턴
...
// GetRvMessage 를 호출한 후, eRvReadType 에 PPBODY, DMS 가 들어 있으면 아래 두 함수를 이용해 Bin 을 얻어가야 한다
...
//-----------------------------------------------------------------------------------------------------------------------------------------------
//  RV CMQueue Listener
//  2016.01.11 kilbum.lee : DQ(Distributed Queue) 기능 추가
//-----------------------------------------------------------------------------------------------------------------------------------------------
            ...
            RV_CAST CastMode, CString cszService, CString cszNetwork, CString cszDaemon )
{
    ...
    m_pSender       = NULL;
    m_pListener     = NULL;
    m_pCMQListener  = NULL;     // 2016.01.11 kilbum.lee : DQ(Distributed Queue) 기능 추가
    m_pFTListener   = NULL;     // 2016.01.11 kilbum.lee : DQ(Distributed Queue) 기능 추가
...
CRvMain::~CRvMain()
{
    ...
    // 2016.01.11 kilbum.lee : FT(Fault-Tolerance) 기능 추가
    if ( m_pFTListener )
    ...
    if( pTransport == NULL )
    {
        ...
        while ( 1 )
        {
            if ( m_Create_Transport( cszDescription ) == FALSE )
            {
                ...
                if ( ++nRetryCnt >= 3 )  // 3회 retry 실시에도 transport 생성 안됨
                {
                    return FALSE;
                }
...
void CRvMain::Set_FT_RV_Info( CString cszGroupName, long nWeight, long nActiveGoalNum, 
                             double fHBInterval, double fPrepareInterval, double fActivateInterval  )
{
    // 2017.04.17 LKB : Rv DQ의 IdentifyID 와 FT의 GroupName에 hostname 추가
    char csHostName[256];
...
{
    // 2017.05.19 LKB : Rv DQ의 IdentifyID 설정
    char csHostName[256];
    if ( gethostname( csHostName, sizeof(csHostName)) < 0 )
    {
...
int CRvMain::GetRvMessage( CString &cszRvMsg,  int &eRvReadType, CString &cszRvMsgBin, int nTimeout /*= 0*/ )
{
    ...
    // 2016.03.02 kilbum.lee : FT(Fault-Tolerance) 기능 추가
    if ( m_ListenFlag == RV_FTLISTENER_ON )
    {
